...
if [[ -f /opt/tuic/v5/rsa.pem ]]; then
    ...
    echo "正在申请证书..."
    echo
    read -p "请粘贴${DOMAIN}源证书 BEGIN CERTIFICATE" csa.pem
    cat >/opt/tuic/v5/rsa.pem <<EOF
${csa.pem}
EOF

read -p "请粘贴${DOMAIN}私钥 BEGIN PRIVATE KEY" key.pem
echo
    cat >/opt/tuic/v5/key.pem  <<EOF
${key.pem}
    ...
    cat >/opt/tuic/v5/tuic.conf <<EOF
{
    ...
    "certificate": "/root/rsa/csa.pem",
    "private_key": "/root/rsa/key.pem",
...
if [[ $(pm2 ls | grep tuic | wc -l) -gt 0 ]]; then
    ...
    echo
    echo "正在启动tuic-server ..."
    pm2 start ./tuic-server -- -c /root/rsa/v5.conf
fi
...
uuids=($(jq -r '.users | keys_unsorted[]' /root/rsa/v5.conf))

for uuid in "${uuids[@]}"; do
  password=$(jq -r ".users[\"$uuid\"]" /root/rsa/v5.conf)
