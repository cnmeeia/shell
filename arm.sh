...
wget https://github.com/EAimTY/tuic/releases/download/0.8.5/tuic-server-0.8.5-aarch64-linux-gnu -O tuic-server && chmod +x tuic-server
...
if [[ $? -eq 0 ]]; then
    ...
    echo "正在启动tuic..."
    pm2 start ./tuic-server -- -c tuic-server.conf
fi
